// TypeScript types for the ORE v0.1 format.
// These mirror spec/schema/*.schema.json; the schema is normative.
...
export type GrowthCurve = number | { fromYear: number; annualPercent: number }[];
...
export interface Property {
  ...
  description?: string;
  address: { street1: string; city: string; state: string; postalCode: string; county?: string; country?: string };
  location?: { latitude?: number; longitude?: number; market?: string; submarket?: string };
  parcels?: { apn: string; landAcres?: number; zoning?: string }[];
...
export interface Lease {
  ...
  baseRent: { unit: RentUnit; schedule: { startDate: string; amount: number }[] };
  escalation?: Escalation;
  reimbursement: Reimbursement;
  freeRent?: { startDate: string; endDate: string; percentAbated?: number; abatesReimbursements?: boolean }[];
  tenantImprovements?: { amountPerSF?: number; totalAmount?: number; fundingDate?: string };
  leasingCommissions?: { amountPerSF?: number; totalAmount?: number; percentOfTotalRent?: number; fundingDate?: string };
  options?: { type: string; [k: string]: unknown }[];
  securityDeposit?: { amount: number; form?: string };
  guarantor?: string;
  notes?: string;
}
...
export interface MarketAssumptions {
  asOfDate?: string;
  growth: { marketRent: GrowthCurve; expenses: GrowthCurve; cpi?: GrowthCurve };
...
export interface LeasingProfile {
  marketRent: { amount: number; unit: "perSFPerMonth" | "perSFPerYear" };
  termMonths: number;
  escalation?: Escalation;
...
export interface DirectCap {
  ...
  markToMarket?: { discountRatePercent: number; notes?: string };
  adjustments?: { name: string; type: "deduction" | "addition"; amount: number; category?: string; notes?: string }[];
  notes?: string;
}
