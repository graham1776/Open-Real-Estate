...
import type {
  ...
  DirectCap, Dcf, Debt, RentUnit, ExpenseItem,
} from "./types.js";
...
interface EngineCtx {
  ...
  taxOverride?: { expenseId: string; baseAnnual: number };
}

interface TaxReassessInfo {
  expenseId: string;
  ratePct: number;
  baseAnnual: number;    // reassessed (going-in) property tax, USD/yr, before growth
  currentAnnual: number; // the file's stated current property tax, USD/yr
  reassessAcq: boolean;
  reassessRev: boolean;
  rateDerived: boolean;
}
...
function resolveTaxReassessment(deal: OreFile): TaxReassessInfo | null {
  ...
  if (!taxItem || price == null) return null;
...
function itemGrowthCurve(deal: OreFile, expenseId: string): GrowthCurve | undefined {
...
interface ExpenseMonth {
  ...
  recoverableControllable: number; // recoverable subset that is controllable (cap-eligible)
  belowLine: number;
...
function isControllable(item: ExpenseItem): boolean {
  if (item.controllable != null) return item.controllable;
...
function expensesAt(ctx: EngineCtx, m: number, excludeIds?: string[]): ExpenseMonth {
  const out: ExpenseMonth = { aboveFixed: 0, recoverableFixed: 0, recoverableControllable: 0, belowLine: 0, mgmtPct: 0, mgmtRecoverable: false };
  ...
  for (const item of ctx.deal.expenses?.items ?? []) {
    ...
    if (ctx.taxOverride && item.expenseId === ctx.taxOverride.expenseId) annual = ctx.taxOverride.baseAnnual; // property tax reassessed to purchase price
    ...
    if (item.belowTheLine) out.belowLine += monthly;
      ...
      if (item.recoverable) {
        ...
        if (isControllable(item)) out.recoverableControllable += monthly;
      }
    }
  }
  return out;
 ...
 */
function capCeilingsAnnual(ctx: EngineCtx, lease: Lease, years: number): number[] {
  ...
  const actual = (y: number) => expensesAt(ctx, (y - 1) * 12, excl).recoverableControllable * 12;
  const out: number[] = [];
    ...
    for (let m = c0; m < Math.min(expiry, horizon); m++) {
      ...
      if ((structure === "NNN" || structure === "NN") && reimbursing) {
        ...
        if (capCeil) {
          ...
          if (excess > 0) mgRecov[m]! -= share * excess * adminMarkup;
        }
      }
...
export function computeDirectCap(deal: OreFile, model: MonthlyModel, warnings: Warnings): DirectCapResult | null {
  ...
  if (dc.excludeExpenseIds?.length) {
    ...
    const ctx: EngineCtx = { deal, start: deal.valuation!.analysisStartDate, buildingSF: sf, horizon: 12, warnings, cpiAnnualPercent: null, taxOverride: rx?.reassessAcq ? { expenseId: rx.expenseId, baseAnnual: rx.baseAnnual } : undefined };
    let excluded = 0;
    for (let m = 0; m < 12; m++) {
...
export function computeDCF(deal: OreFile, model: MonthlyModel, warnings: Warnings): DcfResult | null {
  ...
  if (tv.method === "direct_cap") {
    ...
    if (rx?.reassessRev) {
      ...
      let sellerTax = 0;
      for (let m = from; m < from + 12; m++) sellerTax += (effBase / 12) * growthFactor(curve, m);
...
export function annualTable(model: MonthlyModel, H: number): AnnualRow[] {
...
// --------------------------------------------------------------- portfolio
//
// Aggregates multiple deal files into one roll-up. This is the format paying
// off at the portfolio level: ten deals from ten different producers aggregate
// cleanly because they are all the same format. Combined cash flows sum each
// deal's monthly model through its own hold; portfolio returns combine the
// deals that carry a purchase price and DCF (others are listed as excluded).
...
export function computePortfolio(entries: PortfolioEntry[]) {
  const warnings = new Warnings();
  ...
  const dealHs = per.map((p) =>
    p.deal.valuation?.dcf ? (p.deal.valuation.dcf.holdPeriodMonths ?? p.deal.valuation.dcf.holdPeriodYears! * 12) : 12);
  ...
  const combined = Object.fromEntries(modelKeys.map((k) => [k, new Array<number>(maxH).fill(0)])) as unknown as MonthlyModel;

  // ---- portfolio returns (deals with purchasePrice + computable DCF)
  const cfU = new Array<number>(maxH + 1).fill(0);
  const cfL = new Array<number>(maxH + 1).fill(0);
  let costSum = 0, equitySum = 0, anyDebt = false;
  const includedDeals: string[] = [];
  const excludedDeals: string[] = [];
  ...
  per.forEach((p, i) => {
    ...
    for (const k of modelKeys) {
      for (let m = 0; m < H; m++) combined[k][m]! += model[k][m]!;
    }
    ...
    if (dcfRes && price != null) {
      ...
      costSum += cost;
      cfU[0]! -= cost;
      for (let m = 0; m < H; m++) cfU[m + 1]! += model.cashFlow[m]!;
      cfU[H]! += dcfRes.terminalNet;
      const debt = buildDebt(p.deal, model, H, wLocal);
      const net = debt?.netProceeds ?? 0;
      if (debt) anyDebt = true;
      equitySum += cost - net;
      cfL[0]! -= cost - net;
      for (let m = 0; m < H; m++) cfL[m + 1]! += model.cashFlow[m]! - (debt ? debt.service[m]! : 0);
  ...
  const expir = new Map<number, { sf: number; rent: number }>();
  const tenants = new Map<string, { sf: number; annualRent: number; earliestExpiration: string; deals: Set<string> }>();
  for (const p of per) {
    ...
    for (const lease of p.deal.rentRoll.leases) {
      ...
      const ctx: EngineCtx = { deal: p.deal, start, buildingSF: p.deal.property.physical.buildingSF, horizon: expiryMonth + 1, warnings: new Warnings(), cpiAnnualPercent: null };
      ...
      const t = tenants.get(lease.tenant.name) ?? { sf: 0, annualRent: 0, earliestExpiration: lease.expirationDate, deals: new Set<string>() };
      t.sf += lease.leasedSF;
      t.annualRent += inPlaceRent;
      if (lease.expirationDate < t.earliestExpiration) t.earliestExpiration = lease.expirationDate;
      t.deals.add(p.label);
